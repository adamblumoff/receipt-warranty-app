...
const extractFields = (
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const rangeIndicatorPattern = /thru|through|until|til|till|to|–|—|-/i;
    const hasRangeKeyword = rangeIndicatorPattern.test(line);
    let confidence = scoreForContext(line, [
      'expire',
      'valid',
      'thru',
      'through',
    ...
    if (rangeIndicatorPattern.test(line)) {
      confidence = Math.max(confidence, 0.95);
    } else if (/expire|exp\b/i.test(line)) {
      confidence = Math.max(confidence, 0.9);
    } else if (/redeem/i.test(line)) {
      confidence = Math.max(confidence, 0.88);
    } else if (/valid/i.test(line)) {
    ...
    if (nextLine) {
      const nextHasRange = rangeIndicatorPattern.test(nextLine);
      const nextMatches = Array.from(nextLine.matchAll(DATE_REGEX)) ?? [];
      const nextIsoValues = nextMatches
        .map((match) => parseIsoDate(match[0]))
        .filter((value): value is string => Boolean(value));
      if (nextIsoValues.length > 0 && (nextHasRange || hasRangeKeyword || isoValues.length === 1)) {
        isoValues.push(...nextIsoValues);
        sourceLine = `${line} ${nextLine}`;
        if (!hasRangeKeyword && nextHasRange) {
    ...
    const candidateIso =
      hasRangeKeyword || isoValues.length > 1
    ...
    if (
      ...
      fields.expiresOn = buildField(candidateIso, confidence, sourceLine);
    }
  }
